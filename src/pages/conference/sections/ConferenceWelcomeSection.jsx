...
export default function ConferenceWelcomeSection() {
  ...
  const fetchData = async () => {
    try {
      const result = await apiRequest("/conference-welcome", "GET");
  ...
  const saveToBackend = async () => {
    ...
    try {
      let result;
      if (data.id) {
        result = await apiRequest(`/admin/conference-welcome/${data.id}`, "PUT", data, token);
      } else {
        result = await apiRequest("/admin/conference-welcome", "POST", data, token);
      }
  ...
  if (!token) {
    return (
      <div className="text-center py-20">
        ...
        <p className="mt-2 text-slate-500">Please login to manage conference welcome section.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold">Conference Welcome Section</h2>
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4">
        ...
        <div>
          ...
          <input value={data.subtitle} onChange={(e) => updateField("subtitle", e.target.value)} className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500 outline-none" />
        </div>
        <div>
      ...
      <div className="bg-gray-50 rounded-xl border p-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mt-2">{data.title}</h2>
        <p className="text-slate-500 mt-3 max-w-2xl mx-auto">{data.description}</p>
      </div>
    </div>
  );
}