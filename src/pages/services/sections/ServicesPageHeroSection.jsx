...
export default function ServicesPageHeroSection() {
  ...
  const fetchData = async () => {
    try {
      const result = await apiRequest("/services-page-hero", "GET");
      if (result.success && result.data) {
        setData({
          id: result.data.id || null,
          title: result.data.title || "Services",
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      if (data.id) {
        ...
        result = await apiRequest(`/admin/services-page-hero/${data.id}`, "POST", formData, token, true);
      } else {
        result = await apiRequest("/admin/services-page-hero", "POST", formData, token, true);
      }
  ...
  if (!token) {
    return (
      <div className="text-center py-20">
        ...
        <p className="mt-2 text-slate-500">Please login to manage page hero section.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold">Services Page Hero</h2>
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4">
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">This title will appear in the breadcrumb and as the page heading</p>
        </div>
      ...
      <div className="bg-gray-50 rounded-xl border p-6">
        ...
        <div className="mt-3 text-center text-sm text-slate-500">
          <span className="text-emerald-600">Home</span> / {data.title}
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-xs text-blue-600">
              ...
              <li>• The breadcrumb will show: Home / [Your Title]</li>
