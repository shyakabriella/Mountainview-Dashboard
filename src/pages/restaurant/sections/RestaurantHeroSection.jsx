...
export default function RestaurantHeroSection() {
  ...
  const fetchData = async () => {
    try {
      const result = await apiRequest("/restaurant-hero", "GET");
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      formData.append("subtitle", data.subtitle || "Home / Restaurant");
      ...
      if (data.id) {
        ...
        result = await apiRequest(`/admin/restaurant-hero/${data.id}`, "POST", formData, token, true);
      } else {
        result = await apiRequest("/admin/restaurant-hero", "POST", formData, token, true);
      }
  ...
  if (!token) {
    return (
      <div className="text-center py-20">
        ...
        <p className="mt-2 text-slate-500">Please login to manage restaurant hero section.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold">Restaurant Hero Section</h2>
      ...
      <div className="bg-white rounded-xl border p-6 space-y-4">
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">Main heading that appears on the hero banner</p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">Breadcrumb navigation text</p>
        </div>
          ...
          {data.imagePreview ? (
            <>
              ...
              <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center">
                ...
                <h1 className="text-4xl font-bold text-white mt-2">{data.title}</h1>
              </div>
            </>
          ) : (
            <div className="w-full h-full bg-gradient-to-r from-gray-700 to-gray-900 flex flex-col items-center justify-center">
              <p className="text-white/60 text-sm">{data.subtitle || "Home / Restaurant"}</p>
              <h1 className="text-4xl font-bold text-white mt-2">{data.title || "Restaurant"}</h1>
            </div>
          )}
        </div>
        <div className="mt-3 text-center text-sm text-slate-500">
          <span className="text-emerald-600">Home</span> / Restaurant
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="mt-1 space-y-1 text-xs text-blue-600">
              ...
              <li>• The title will appear centered over the image with a dark overlay</li>
              <li>• The subtitle appears above the title as a breadcrumb</li>
